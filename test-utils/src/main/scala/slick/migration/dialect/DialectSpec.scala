...
abstract class DialectSpec(confName: String, driver: JdbcDriver) extends FlatSpec with BeforeAndAfterAll  {
  val db = new DbInit(confName, driver)

  import db.driver.simple._

  implicit val dialect: Dialect[_]

  override def beforeAll() { db.cleanUpBefore() }
  override def afterAll()  { db.cleanUpAfter()  }

  behavior of s"$confName dialect for slick's migration"

  it should "create new table" in new TableTesting {
    val table = "test_create_table"
    val migration = initMigration

    getTables should have size 0

    db.conn withSession { implicit s => migration.create.addColumns(_.id, _.text).apply }
  ...
  it should "drop existing table" in new TableTesting {
